var GameObjManager = Class.extend({
    init: function(){
        this.WindowWidth = 800; //window.innerWidth;
        this.WindowHeight = 480; //window.innerHeight;
        this.CurrentGame = new Game(this);
        this.IndexButtons = [];
        this.Players = localStorage['players'] === null ? [] : JSON.parse(localStorage['players']);
        this.CurrentPlayer = {};
    },
    //
    //Draw Methods
    ...
    ButtonClick: function(x, y){
        ...
        for (var i = 0, len = buttons.length; i < len; i++) {
            ...
            var top = buttons[i].y,
                bottom = buttons[i].y + buttons[i].height;
    ...
    SavePlayer: function(player){
        var players = localStorage['players'] === null ? [] : JSON.parse(localStorage['players']);
